...
void crearAnimal(string codigoPropietario,vector<Animal*>&arrayMascotas,vector<Perro>&arrayPerros,vector<Gato>&arrayGatos){
  system("cls");
  int especie;
  string raza;
  string nombre;
  ...
  cin>>sexo;
  if(especie-1){
    Gato nuevoG(codigoPropietario,raza,nombre,color,day,month,year,sexo);
    arrayGatos.push_back(nuevoG);
    arrayMascotas.push_back(&arrayGatos[arrayGatos.size()-1]);
  }else{
    Perro nuevoP(codigoPropietario,raza,nombre,color,day,month,year,sexo);
    arrayPerros.push_back(nuevoP);
    arrayMascotas.push_back(&arrayGatos[arrayPerros.size()-1]);
  }
  return;
}
...
void cargarPropietarios(vector<Propietario>&arrayPropietarios){
...
void cargarPerros(vector<Perro>&arrayPerros){
  ifstream nuevo("TodoslosPerros.txt");
  int cantidad;
  nuevo>>cantidad;
  nuevo.ignore();
  for(int i=0;i<cantidad;i++){
    ...
    nuevo>>sexo;
    nuevo.ignore();
    Perro nuevoP(codigoDelPropietario,raza,nombre,color,day,month,year,sexo);
    arrayPerros.push_back(nuevoP);
  }
}
void cargarGatos(vector<Gato>&arrayGatos){
  ifstream nuevo("TodoslosGatos.txt");
  int cantidad;
  nuevo>>cantidad;
  nuevo.ignore();
  for(int i=0;i<cantidad;i++){
    ...
    nuevo>>sexo;
    nuevo.ignore();
    Gato nuevoG(codigoDelPropietario,raza,nombre,color,day,month,year,sexo);
    arrayGatos.push_back(nuevoG);
  }
}
void cargarAnimales(vector<Animal*>&arrayMascotas,vector<Perro>&arrayPerros,vector<Gato>&arrayGatos){
  for(int i=0;i<arrayPerros.size();i++){
    arrayMascotas.push_back(&arrayPerros[i]);
  }
  for(int i=0;i<arrayGatos.size();i++){
    arrayMascotas.push_back(&arrayGatos[i]);
  }
}
void guardarPropietaros(vector<Propietario>&arrayPropietarios){
...
void guardarPerros(vector<Perro>&arrayPerros){
  ofstream guar("TodoslosPerros.txt");
  guar<<arrayPerros.size()<<endl;
  for(int i=0;i<arrayPerros.size();i++){
    guar<<arrayPerros[i].getCodigoDelPropietario()<<endl;
    guar<<arrayPerros[i].getRaza()<<endl;
    guar<<arrayPerros[i].getNombre()<<endl;
    guar<<arrayPerros[i].getColor()<<endl;
    guar<<arrayPerros[i].getNacimiento(0)<<endl;
    guar<<arrayPerros[i].getNacimiento(1)<<endl;
    guar<<arrayPerros[i].getNacimiento(2)<<endl;
    guar<<arrayPerros[i].getSexo()<<endl;
  }

}
void guardarGatos(vector<Gato>&arrayGatos){
  ofstream guar("TodoslosGatos.txt");
  guar<<arrayGatos.size()<<endl;
  for(int i=0;i<arrayGatos.size();i++){
    guar<<arrayGatos[i].getCodigoDelPropietario()<<endl;
    guar<<arrayGatos[i].getRaza()<<endl;
    guar<<arrayGatos[i].getNombre()<<endl;
    guar<<arrayGatos[i].getColor()<<endl;
    guar<<arrayGatos[i].getNacimiento(0)<<endl;
    guar<<arrayGatos[i].getNacimiento(1)<<endl;
    guar<<arrayGatos[i].getNacimiento(2)<<endl;
    guar<<arrayGatos[i].getSexo()<<endl;
  }

}
void MenuInscribir(vector<Propietario>&arrayPropietarios,vector<Animal*>&arrayMascotas,vector<Perro>&arrayPerros,vector<Gato>&arrayGatos){
  system("cls");
  if(SiNo("El Propietario ya ha sido registrado? s/n:")){
    ...
    int rpta=OpcionesLista(Resultados.size(),"Opcion: ");
    crearAnimal((*Resultados[rpta]).getCodigo(),arrayMascotas,arrayPerros,arrayGatos);
  }else{
    cout<<"inscribiendo propietario: "<<endl;
    Propietario nuevo=crearPropietario(arrayPropietarios);
    crearAnimal(nuevo.getCodigo(),arrayMascotas,arrayPerros,arrayGatos);
